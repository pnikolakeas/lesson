...
public class PresentationsGridItem extends BasicGridItem {
	
	public PresentationsGridItem(String text, TreePanelDropTargetExtended dropTarget) {
		super(text);
	...
	protected List<ColumnConfig> initColumnConfigs() {
		...
		column = new ColumnConfig("presentedBy", "BY WHOM", 276);
		column.setEditor(new CellEditor(text));
		configs.add(column);
		...
		column = new ColumnConfig("keyword", "KEYWORD", 276);
		column.setEditor(new CellEditor(text));
		configs.add(column);
